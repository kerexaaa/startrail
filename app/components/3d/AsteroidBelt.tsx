...
const INNER_RADIUS = SUN_RADIUS + 2.1 * DISTANCE_SCALE; // ~2.1 AU
const OUTER_RADIUS = SUN_RADIUS + 3.3 * DISTANCE_SCALE; // ~3.3 AU
const BELT_HEIGHT = 1.5; // vertical spread
const MIN_ASTEROID_SIZE = 0.04;
const MAX_ASTEROID_SIZE = 0.15;

// Average orbital period ~4.5 years for main belt asteroids
const BELT_SPEED = (1 / 4.5) * BASE_SPEED;

// Pre-generate deterministic pseudo-random asteroid data
function generateAsteroids() {
  ...
  for (let i = 0; i < ASTEROID_COUNT; i++) {
    // Use a simple hash-like distribution for deterministic results
    ...
    // Vary speed slightly per asteroid (Kepler's 3rd law approximation)
...
export default function AsteroidBelt() {
  ...
  // Reset sim time when user resets time
  useEffect(() => {
    simTime.current = 0;
  }, [timeResetTrigger]);
